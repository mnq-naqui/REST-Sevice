...
public class BankAccountControllerTest extends BankAccountServiceApplicationTests {
	...
	public void testDeleteBankAccount() throws Exception {
		mockMvc.perform(delete("/bankaccount/{id}", id))
	...
	public void testUpdateBankAccount() throws Exception {
		...
		mockMvc.perform(put("/bankaccount/{id}",id)
		.content(asJsonString(getUserResource()))
	...
	public static UserDetailResource getUserResource() {
		...
		ArrayList<BankResource> bankList=new ArrayList<>();
		ArrayList<BranchResource> branchList=new ArrayList<>();
		...
		branchList.add(branchResource);
		
		resource.setBanks(bankList);
