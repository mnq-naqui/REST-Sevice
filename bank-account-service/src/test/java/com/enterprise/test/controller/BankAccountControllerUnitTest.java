...
public class BankAccountControllerUnitTest{
	...
	public void testGetBankAccounts() throws Exception {
		...
		Link banksLink = new Link(URL.BASE_PATH+URL.GET_BANKACCOUNT+"/"+userResource.getUserId()+"/banks").withRel("banks");
		...
		mockMvc.perform(get(URL.GET_BANKACCOUNT)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$..userId").value(userResource.getUserId())).andExpect(jsonPath("$..userName").value(userResource.getUserName()))
				.andExpect(jsonPath("$..email").value(userResource.getEmail())).andExpect(jsonPath("$..mobile").value(userResource.getMobile()))
				.andExpect(jsonPath("$..stateRef").value(userResource.getStateRef())).andExpect(jsonPath("$..cityRef").value(userResource.getCityRef()))
				.andExpect(jsonPath("$..permanentAddress").value(userResource.getPermanentAddress())).andExpect(jsonPath("$..currentAddress").value(userResource.getCurrentAddress()))
	...
	public static UserDetail getUserDetail() {
		...
		return  new UserDetail(1, "Naqui Qureshi", "naqui.qureshi@evry.com", "900842788", "HSR Bangalore", "Chhindwara", 1, 2, "560101");
	}
